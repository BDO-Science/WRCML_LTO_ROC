spatTempAgg <- function(df, region, subregion, station, study, 
                        variable, stat = NULL, 
...
spatTempAgg_makeDF <- function(file, list = F, pbar = T) {
  ...
  inputs <- read.csv(file) %>% 
    mutate(across(c(region, subregion, station, variable, stat, spatAggFun, timeAggFun), ~as.character(.x)),
           across(c(timeAggLength, timeLag), ~as.double(.x)),
  ...
  if (pbar) {
    ...
    df <- pbmapply(spatTempAgg, 
                   ...
                   MoreArgs = list(df = data$joinedNew),
                   SIMPLIFY = F)
  } else {
